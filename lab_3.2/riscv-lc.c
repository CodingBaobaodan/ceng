...
void latch_datapath_values() {
    ...
    if (get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION)) {
         ...
         *  Lab3-2 assignment
         */
        if(get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 0)
            NEXT_LATCHES.PC = CURRENT_LATCHES.PC + 4;
        else if(get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1)
            NEXT_LATCHES.PC = BUS + CURRENT_LATCHES.PC - 4;  
    }
    /* RESET */
