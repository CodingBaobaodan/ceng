...
void handle_slli(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), 
        ...
        imm4 = MASK24_20(cur_inst);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] << imm4;
}
...
void handle_srai(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), 
        rs1 = MASK19_15(cur_inst), 
        imm5 = MASK24_20(cur_inst);
    //in C, the operator ">>" will perform arithmetic shift on a signed operand 
...
void handle_lui(unsigned int cur_inst) {
    ...
    NEXT_LATCHES.REGS[rd] = sext(imm20 << 12, 32);
}
...
void handle_sll(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst),
        rs1 = MASK19_15(cur_inst),
        rs2 = MASK24_20(cur_inst);
    //masking the lower 5 bits in rs2 first, then perform shifting operation
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] << (0x1F & CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_srl(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst),
        rs1 = MASK19_15(cur_inst),
        rs2 = MASK24_20(cur_inst);
    //masking the lower 5 bits in rs2 first, then performs the shifting operation
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] >> (0x1F & CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_lh(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst);
    int base_address = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = sext(MASK15_0(MEMORY[base_address] + (MEMORY[base_address + 0x01] << 8)), 16);
}
...
void handle_lw(unsigned int cur_inst) {
    ...
    int imm12 = MASK31_20(cur_inst);
    int base_address = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = sext(MEMORY[base_address] + (MEMORY[base_address + 0x01] << 8) + (MEMORY[base_address + 0x02] << 16) + (MEMORY[base_address + 0x03] << 24), 32);
}
...
void handle_sb(unsigned int cur_inst) {
    ...
    int base_address = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    MEMORY[base_address] = sext(MASK7_0(CURRENT_LATCHES.REGS[rs2]), 8);
    if(MASK7_0(CURRENT_LATCHES.REGS[rs2]) >> 7){ //if we store a negative value
        MEMORY[base_address + 0x01] = 0xFF;
        MEMORY[base_address + 0x02] = 0xFF;
        MEMORY[base_address + 0x03] = 0xFF;
    }
...
void handle_sh(unsigned int cur_inst) {
    ...
    int imm12 = MASK11_7(cur_inst) + (MASK31_25(cur_inst) << 5);
    int base_address = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    MEMORY[base_address] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base_address + 0x01] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
    if(MASK15_8(CURRENT_LATCHES.REGS[rs2]) >> 7){ //if we store a negative value
        MEMORY[base_address + 0x02] = 0xFF;
        MEMORY[base_address + 0x03] = 0xFF;
    }
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[base_address] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base_address + 0x01] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base_address + 0x02] = MASK23_16(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base_address + 0x03] = MASK31_24(CURRENT_LATCHES.REGS[rs2]);
